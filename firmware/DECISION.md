...
**DIY board first.** It is the only path that reaches GoFar parity at US$15–30/unit
...
| Criterion | ESP32 DIY (US$15–30) | Freematics ONE+ B (US$135) | Hybrid (iCar Pro + app) |
|---|---|---|---|
| Phone-dead trip capture | ✅ on-board LittleFS | ✅ on-board SD | ❌ dies with phone |
| Cost/unit (product scale) | **US$15–30** | US$135 | +$25 adapter + app |
...
- `firmware/esp32-diy/` — TWAI CAN ignition detect → OBD-II PID sampling (RPM/speed)
  → LittleFS CSV per trip → DS3231 RTC epochs → deep sleep (timer + ACC GPIO wake)
  → BLE service advertising trip index. **Compile-verified** (PlatformIO) + host
  self-check (`test/self_check.cpp`) for the pure PID/CSV/RTC logic.
...
- Row schema `epoch,rpm,speed,coolant,throttle` shared by both paths so the app
  sync code is written once.

## Remaining to close the decision (hardware in hands)

1. Nathan: prototype the DIY board (PCB + MCP2551 + DS3231 + buck) — 1–2 wks.
