...
Instances (Demo, Default, Hosted) are upgraded automatically through the
**upgrade path** — a deterministic, Portainer-API redeploy of each tier's stack
in the mandated promotion order (Demo → Default → Hosted, per AUT-107), with
`pullImage` so the freshly built images are pulled and changed services
...
2. On completion, the `deploy-instances.yml` workflow runs
   `scripts/upgrade-instances.sh`, which redeploys each stack via
   `PUT /api/stacks/{id}?endpointId={ep}` (Portainer 2.45 — the `/redeploy`
   sub-route does not exist here) with the stack's own compose + env and
   `Prune: false`, preserving volumes and config.
...
non-secret vars so a redeploy can never fail at interpolation again.