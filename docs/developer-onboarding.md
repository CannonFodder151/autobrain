...
```bash
git clone git@github.com:<you>/autobrain.git
cd autobrain
```
...
frontend/  Flutter app                     (lib/ = package)
...
- Add an endpoint → `backend/app/api/v1/<area>.py` + `schemas/<area>.py`,
  register in `api/v1/__init__.py`.
- Add a table → model in `backend/app/models/`, autogenerate migration.
- Add an AI feature → module in `ai/app/modules/` + fallback in
  `ai/app/fallbacks.py` + client fn in `backend/app/services/ai_client.py`.
...
Behaviour changes update BOTH the Outline wiki and `docs/` mirror.

## 8. Rego / market data providers

External lookups (`REGO_LOOKUP_URL`, `MARKET_DATA_URL`) are optional; see
`.env.example`. Without them the app uses deterministic offline heuristics.
