...
`mod_impact.py`, `fuel_ocr.py`, `odometer.py`, `parts_guide.py`, `condition.py`).
Each exports a single `*_fallback()` entry point used by the matching
`ai/app/modules/` handler.
