...
- **Parts — SCA lookup** (`/vehicles/{id}/parts/sca-lookup`): GET with `{rego, state, make, model, year, refresh}` → resolves the vehicle (rego+state via rego-lookup, else make/model/year), scrapes the Supercheap Auto parts-guide via the market-data container, and returns **Inventory-formatted** parts (`name`, `category`, `sku`, `supplier`, `brand`, `description`, `service_group`) normalised + 9Router-tidied. Cached 24h in `sca_parts_cache`.
- **Parts — service prefill** (`/vehicles/{id}/parts/suggest-for-service`): POST `{service_type, rego?, state?, refresh?}` → returns `items` (ServiceItemIn shape) for an AI-suggested service, **preferring parts already in the user's inventory** whose category matches the service, then Supercheap Auto suggestions for that service type.
