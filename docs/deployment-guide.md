...
| 2026-08-13 | v0.3.55 | Demo-only reseed deploy of AUT-529 demo build photos (PR #95, merge `09a8d365`) to the Demo tier (Portainer stack 73 EP2, `pullImage:true`, images `cannonfodder151/autobrain-backend`/`-ai`: `0.3.55`, worker/beat `:latest`, frontend ghcr `:demo`). One-time `DEMO_RESET=true` recreate: backend logs `demo_reset_deleted` + `demo_seed_created`, 0 `demo_build_photo_seed_failed`; MinIO `autobrain-assets/demo/` = 12 build PNGs (4 builds × 3). Then `DEMO_RESET=false` + restart — boot logs `demo_seed_already_exists`, no re-seed. Default + Hosted untouched (Demo-only, AUT-538 scope; v0.3.55 already live there per AUT-582). | `/health` → 0.3.55 on demo; `/api/v1/social/feed` (demo@autobrainservice.app) → 4 demo builds (Skyline R34, Weekend MX-5, Project Silvia, Ducati Monster) each with exactly 3 photos |
