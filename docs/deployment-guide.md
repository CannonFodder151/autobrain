...
|------|---------|--------|----------|
| 2026-08-17 | v0.3.94 | Post-merge deploy of AUT-997 deleted-post tombstone fix (PR #176, merge `e70a270`): `_tombstone_removed_build()` in `backend/app/api/v1/social.py` writes a federation tombstone on every author/admin delete so community-garage builds stop re-appearing in the feed after inbox re-sync; wired into `delete_post` (social.py) + admin takedown (admin.py). Original publish run 31983686003 (for `e70a270`) was cancelled and `:0.3.92` on Hub predates the merge, so the fix shipped in v0.3.94 (current main). Multi-arch `0.3.94` manifest was still pending CI assembly, so EP2 stacks pinned `:0.3.94-amd64` and EP5 `:0.3.94-arm64`. Deploy order Demo (73) → Default (68) → Hosted (83), backend+worker/beat only. | `/health` → `0.3.94` on all three tiers (demo/default EP2, hosted EP5); tombstone verified on hosted: delete post → absent from feed, `social_remote_tombstones` row written for the local build id, simulated hub re-route through `_sync_federation` did NOT resurrect the post (PASS); deployed containers confirmed to contain `_tombstone_removed_build` |
| 2026-08-15 | — | AUT-718 demo seed + DEMO_RESET flip-back (PR #145): demo stack (73 EP2) seeded 16 issue-blog posts with replies via `DEMO_RESET=true` (seed ran 2026-08-14T14:20Z), then `DEMO_RESET` flipped to `"false"` so backend restarts stop wiping/reseeding the demo DB (`reset_demo` only runs on `DEMO_RESET=true`). Redeployed stack 73 with corrected env; demo data survived (no re-wipe). | `/health` ok (0.3.78); login `demo@autobrainservice.app/demo` then `GET /api/v1/social/issues` → 16 posts, 31 replies; stack 73 env `DEMO_RESET="false"` |
| 2026-08-14 | v0.3.62 | Post-merge deploy of IAP verification backend (AUT-617, PRs #115/#117; config auto-bumped past v0.3.61/#122 so images report v0.3.62). `:0.3.61` was never published (changelog-only PR #122 skipped publish) — demo/dev stacks 73/68 moved to `:latest` and Hosted 83 pulled `:hosted` (per correction on AUT-656), `pullImage:true`. Added `POST /billing/iap/verify` (auth), `GET /billing/iap/catalog` (public), `POST /billing/iap/webhook/{apple,google}`; alembic `q7r8s9t0u1v2` adds `users.iap_*` columns, applied via bootstrap on boot. No store credentials set → IAP disabled (empty `IAP_*`). Pruned dangling images (EP2 1.96GB, EP5 2.83GB). | `/health` → `0.3.62` on all three tiers; `GET /billing/iap/catalog` → `{"enabled":false,"products":[...]}` (8 products, disabled) on all three; `POST /billing/iap/verify` → 401 unauthenticated on all three; `users` table has `iap_status`/`iap_product_id`/`iap_platform`/`iap_expires_at`/`iap_purchase_token`/`iap_transaction_id`/`iap_original_transaction_id`/`iap_refreshed_at` on default (EP2) + hosted (EP5) DBs; bootstrap log `alembic_migrations_applied` on both. |
...
| 2026-08-13 | v0.3.42 | Post-merge deploy of AUT-502 Community Garage settings 3-dot menu (PR #88, merge `bbb7897`): admin Settings moved out of the tab bar into the AppBar 3-dot menu; non-admins see no menu. Deploy order Demo → Default → Hosted (AUT-107). Backend/ai/market-data/frontend pinned to `:0.3.42` (demo `:0.3.42`/ghcr `:demo` stays, default frontend `:default`, hosted frontend `:0.3.42`). No schema changes in this release. | `/health` 0.3.42 on all three tiers; served `main.dart.js` on all tiers contains 3-dot menu marker (`Community Garage options`) + admin-only gating; demo API smoke: login, `GET /social/feed` + `/social/my-posts` OK; prune: EP2/EP5 0 reclaimed |
