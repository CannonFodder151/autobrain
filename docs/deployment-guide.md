...
| 2026-08-13 | env fix | AUT-532 federation hub compose env fix (PR #101, merge `8706d641`): added `SOCIAL_FEDERATION_HUB_URL` + `SOCIAL_FEDERATION_HOSTED` to the `backend` service of all stacks. Hosted (EP5 stack 83) already carried the lines (redeployed concurrently at 0.3.53 — env live, `hosted=true`); Demo/Default (EP2 stacks 73/68) updated in Portainer + redeployed (`pullImage:true`); dev-box garage spokes (EP6) already had the env. No image rebuild — compose env wiring only. | `/health` ok on demo/default/hosted; `docker inspect` shows `SOCIAL_FEDERATION_HUB_URL=https://hub.autobrainservice.app` + `SOCIAL_FEDERATION_HOSTED` on all backend containers; hub `POST /v1/register` from hosted backend → 422 (validation, NOT 502 `hub not configured`) |
