...
### Fixed
- **Mobile app sync workflow (AUT-1939):** sync-mobile now auto-detects `invalid_override` analyze failures and self-heals mock method signatures to match the current `ApiClient` interface, preventing the release pipeline from stalling when frontend API signatures change.
