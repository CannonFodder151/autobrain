...
### Added (AUT-2406)
- feat(fuel): SA SAFPIS ingester (Informed Sources / Fuel Pricing Information Scheme). Adds `ingest_sa_fuel()` to `backend/app/services/fuel_feeds.py` reusing the QLD DirectAPI parsers; prices are in tenths-of-a-cent (1356.0 → 1.356 $/L) with `Price=9999.0` sentinel dropped. Wired into `ingest_all_fuel()` and Celery task `ingest_fuel_sa`. Config fields: `FUEL_SA_API_KEY`, `FUEL_SA_API_URL`, `FUEL_SA_GEO_REGION_ID=4`, `FUEL_SA_ENABLED`. `docker-compose.prod.yml` + `docker-compose.hosted.yml` pass the new env/secret-file; `scripts/seed-secrets.sh` maps `FUEL_SA_API_KEY` → `fuel_sa_api_key`. Tests in `backend/tests/test_aut_sa_fuel_ingester.py` (6 DB-free cases). Blocked on: `FUEL_SA_API_KEY` secret not yet provisioned (tracked via AUT-2373).
