...
### Fixed (AUT-2598)
- ci(security): compose-config-diff rename detector now ignores same-family fuzzy matches like `FUEL_SA_API_KEY` vs `FUEL_QLD_API_KEY` (region variants of the same FUEL_*_API_KEY pattern, NOT a typo rename). The 1-edit backstop remains for true renames like `AUTOBRIAN_BACKEND_URL` → `AUTOBRAIN_BACKEND_URL`.
