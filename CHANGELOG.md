...
## [Unreleased]
### Fixed (AUT-2352)
- fix(frontend): implement the boot-time `AppConfig.validate()` referenced by `MisconfiguredBackendScreen`. Adds `AppConfig.lastValidationOk` / `lastValidationError` statics and a `Future<void> validate({Duration timeout = 5s})` probe against `<apiOrigin>/healthz` via `package:http` (already a dependency). `main.dart` now runs `await AppConfig.validate()` after `AppConfig.load()` outside `kDebugMode` and mounts `MisconfiguredBackendScreen` (with `return`) on failure, satisfying AC#2 fail-fast and clearing the QA-cited `flutter analyze` errors.
