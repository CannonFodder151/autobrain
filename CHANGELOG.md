...
### Added (AUT-2438)
- mobile+web: powertrain picker (ICE / EV / HEV / PHEV) on the vehicle add and edit screens. Backend: `VehicleCreate` schema now accepts `powertrain` (defaults to `ICE`, already on `Vehicle` model per AUT-2434). The home feature grid gates feature tiles by powertrain: `Fuel` is hidden entirely on EV; `Electric Spy` and `Electricity` logbook tiles appear for EV and PHEV; ICE/HEV keep fuel features visible. Feature-flag gating is pure (`PowertrainType.isElectric`) so no ICE-only UI leaks onto an EV vehicle. Parent: AUT-2420.
...
### Added (AUT-2436)
- mobile+web: electricity log book (EV charging tracker). New `electricity_logs` table mirrors `fuel_logs` 1:1: kWh in, $/kWh, total cost, is_full_charge, optional receipt, plus chained `distance_km` / `km_per_kwh` / `cost_per_km` between consecutive full charges. Backend: model, Pydantic schemas (`ElectricityLogCreate/Update/Out/Stats`), service (`recompute_efficiency` + `compute_electricity_stats`), router at `GET/POST/PATCH/DELETE /vehicles/{id}/electricity` and `GET .../electricity/stats`, alembic migration `aut2436_electricity_logs` (guards `electricity_logs` table create for the create_all fallback). Frontend: `ElectricityLog` model, `ElectricityScreen` (summary card + fl_chart km/kWh line + history list with edit/delete) and `AddElectricityScreen` (date/odo/kWh/price/total/notes/full-charge form). Feature tile added to `_FeatureGrid` for every vehicle. Vehicle `add_event("charge", ...)` records a timeline entry; `sync_odometer` keeps the odometer current. Deterministic only (no AI). Tests: `backend/tests/test_electricity_aut2436.py` (5 cases) + `frontend/test/electricity_model_test.dart`. Parent: AUT-2420.
### Fixed (AUT-2436)
- ci: `aut2436_electricity_logs` migration promoted to a merge revision (`down_revision = ("aut2434_vehicle_powertrain", "aut2375_fuel_history_index")`) to re-unify the two open alembic heads on `main` and unblock the `pytest — model metadata + alembic heads` gate. Branch rebased onto current `main` (add85203) to pick up `aut2375_fuel_history_index`, `aut2381_fuel_source_arbitration`, and intervening changes.
