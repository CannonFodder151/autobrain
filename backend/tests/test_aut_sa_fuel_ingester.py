...
# Prices in tenths of a cent (SAFPIS v1.2). 9999.0 c/L = product unavailable.
SA_DIRECT_PRICES = {"S": [
...
def test_parse_sa_brands_fuels_and_regions() -> None:
...
def test_parse_sa_direct_prices_tenths_of_a_cent_to_cpl_and_drops_unavailable() -> None:
    ...
    # SA fuel 1 = Unleaded 91, SA fuel 2 = P95, SA fuel 3 = Ethanol, SA fuel 4 = Diesel
    ...
    assert ("E10", 9999.0) not in pairs_54321  # 99990 tenths -> 9999.0 c/L sentinel (dropped)
    assert "E10" not in {ft for (ft, _, _) in prices["54321"]}  # ethanol unavailable at 54321
    pairs_54322 = {(ft, price) for (ft, price, _) in prices["54322"]}
    assert ("91", 1390.0) in pairs_54322
    assert ("Diesel", 1502.0) in pairs_54322
