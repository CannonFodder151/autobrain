"""Tests for the fuel-price services (7-Eleven + NSW Fuel API).

All tests are deterministic with no real network: 7-Eleven stubs its fetch and
exercises pure parse/geo logic; NSW exercises pure parsing and the once-per-day
per-instance quota guard.
"""
...
from app.services import fuel_prices as fp  # noqa: E402
...
# ---------------------------------------------------------------------------
# NSW Fuel API (AUT-1813) fixtures + tests
# ---------------------------------------------------------------------------
...
def test_normalise_joins_prices_to_stations_and_skips_unknown():
    rows = fp._normalise(SAMPLE_PAYLOAD)
...
def test_poll_due_gate_enforces_once_per_day():
    ...
    # no prior poll -> due
    assert fp.poll_due(None, hours=24) is True
    # within 24h -> not due
    assert fp.poll_due(now - timedelta(hours=23), hours=24) is False
    # exactly 24h+ -> due
    assert fp.poll_due(now - timedelta(hours=24, minutes=1), hours=24) is True
