...
Adds fuel_prices (cached NSW Fuel API snapshot, one row per station+fuel type)
and fuel_price_poll_state (per-instance last-poll timestamp enforcing the
...
down_revision: Union[str, Sequence[str], None] = ("h1i2j3k4l5m6", "h1i2j3k4l5m7")
...
def upgrade() -> None:
    if not _has_table("fuel_prices"):
        ...
        op.create_index("ix_fuel_prices_state", "fuel_prices", ["state"])
        op.create_index("ix_fuel_prices_station_code", "fuel_prices", ["station_code"])
        op.create_index("ix_fuel_prices_fuel_type", "fuel_prices", ["fuel_type"])
        op.create_unique_constraint(
            "uq_fuel_price_station_fuel", "fuel_prices", ["state", "station_code", "fuel_type"]
        )
    if not _has_table("fuel_price_poll_state"):
        op.create_table(
...
def downgrade() -> None:
    if _has_table("fuel_prices"):
        if "uq_fuel_price_station_fuel" in {c["name"] for c in sa.inspect(op.get_bind()).get_unique_constraints("fuel_prices")}:
            op.drop_constraint("uq_fuel_price_station_fuel", "fuel_prices", type_="unique")
        op.drop_table("fuel_prices")
    if _has_table("fuel_price_poll_state"):
        op.drop_table("fuel_price_poll_state")
