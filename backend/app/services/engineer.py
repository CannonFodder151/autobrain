...
# These are major city centroids; real implementation should use a postcode DB.
POSTCODE_CENTROIDS = {
...
async def search_engineers(
    ...
    if filters.q:
        ...
        # Specialties and certifications are JSONB arrays - search with ? operator
        # Note: PostgreSQL JSONB containment @> for arrays requires exact element match
        # We use a text search fallback for simplicity
    ...
    if filters.specialties:
        for spec in filters.specialties:
            # Use JSONB contains operator for array membership
    ...
    if filters.available_day is not None or filters.available_from is not None or filters.available_to is not None:
        ...
        if filters.available_day is not None:
            # Check if any availability entry matches the day (1=Mon..7=Sun -> 0=Mon..6=Sun)
    ...
    if sort == "distance" and center_lat is not None and center_lon is not None:
        # Order by haversine distance computed in SQL
        distance_expr = text(
            f"6371 * 2 * asin(sqrt("
...
# Backfill helper (called from migration or admin CLI)
async def backfill_engineer_embeddings(db: AsyncSession) -> int:
    """Generate embeddings for engineers (specialties + certifications text)."""
    from app.services.vector_search import generate_embedding

    engineers = (await db.execute(select(Engineer).where(Engineer.embedding.is_(None)))).scalars().all()
    count = 0
